...
const signup = async (req, res) => {
    try {
        ...
        if (existingUser) {
            // Update the existing user's password and other details
            const saltrounds = 10;
            const hashedPassword = await bcrypt.hash(password, saltrounds);
            await existingUser.update({ name, phonenumber, password: hashedPassword });
            return res.status(200).json({ message: 'User details updated successfully. Please log in with your new password.' });
        }
